...
//        return view('website.index3');
    ...
    {
        if ($category == 'porojo'){
            $porojo = Post::where('post_category_id','1')->orderBy('post_date','desc')->get();
        }elseif ($category == 'domokaya'){
            $porojo = Post::where('post_category_id','2')->orderBy('post_date','desc')->get();
        }elseif ($category == 'porojo_live'){
            $porojo = Post::where('post_category_id','3')->orderBy('post_date','desc')->get();
        }elseif ($category == 'mjue_maarufu'){
            $porojo = Post::where('post_category_id','4')->orderBy('post_date','desc')->get();
        }elseif ($category == 'video'){
            $porojo = Post::where('post_category_id','5')->orderBy('post_date','desc')->get();
        }elseif ($category == 'riwaya'){
            $porojo = Post::where('post_category_id','6')->orderBy('post_date','desc')->get();
        }elseif ($category == 'kingo_katuni'){
            $porojo = Post::where('post_category_id','7')->orderBy('post_date','desc')->get();
        }elseif ($category == 'methali'){
            $porojo = Post::where('post_category_id','8')->orderBy('post_date','desc')->get();
        }
    ...
    public function myPost($id,$name)
    {
        $post = Post::findOrFail($id);
        $post->views = $post->views + 1;
        $post->save();
